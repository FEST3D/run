...
GridDir= 'Mesh'
NumberOfBlocks = 1
AbsBinaryPath="/home/jatinder/solver/FEST3D/bin/FEST3D"

def SetInput(Control, Scheme, Flow, OutputControl, ResidualControl):
    Control['CFL'] = 1.0
    Control['LoadLevel'] = 0
    Control['MaxIterations'] = 100000
    ...
    Scheme['TimeStep']=' g 1e-4'
    Scheme['TimeIntegration']='RK4'
    Scheme['HigherOrderBC']='0'
    
    Flow["NumberOfVariables"] = 5
    Flow["DensityInf"] = 1.176
    Flow["UInf"] = 173.59
    Flow["VInf"] = 0.0
    Flow["WInf"] = 0.0
    ...
    Flow["TKEInf"] = 0.0
    Flow["OmegaInf"] = 0.0
    Flow["ReferenceViscosity"] = 0.2049e-3
...
def SetExpectedInput(ExpectedControl, ExpectedScheme, ExpectedFlow, ExpectedOutputControl, ExpectedResidualControl):
    ...
    ExpectedScheme['InviscidFlux'] = ['ausm', 'ldfss0', 'slau', 'van_leer']
    ...
    ExpectedScheme['TimeIntegration']=['RK4', 'RK2', 'TVDRK2', 'TVDRK3', 'implicit', 'none']
...
def WriteFlowFile(Flow, RootDir):
    with open(RootDir+"/system/flow.md", "w+") as file:
        ...
        file.write("## Free Stream TKE\n")
        file.write(str(Flow['TKEInf'])+"\n\n")
        file.write("## Free Stream Omega\n")
        file.write(str(Flow['OmegaInf'])+"\n\n")
